...
type EditSubscriptionArgs = { id: string; name?: string; price?: number; category?: string; renewalDate?: string };
...
const typeDefs = gql`
  ...
  type Mutation {
    addSubscription(name: String!, price: Float!, category: String!, renewalDate: String!): Subscription!
    editSubscription(id: ID!, name: String, price: Float, category: String, renewalDate: String): Subscription!
...
const resolvers = {
  Query: {
    ...
    subscription: (_: unknown, { id }: SubscriptionArgs): Subscription | undefined => subscriptions.find(sub => sub.id === id),
    categories: (): string[] => [...new Set(subscriptions.map(sub => sub.category))]
  },
  Mutation: {
    addSubscription: (_: unknown, { name, price, category, renewalDate }: AddSubscriptionArgs): Subscription => {
      const newSub: Subscription = { id: String(Date.now()), name, price, category, renewalDate };
      subscriptions.push(newSub);
      return newSub;
    },
    editSubscription: (_: unknown, { id, name, price, category, renewalDate }: EditSubscriptionArgs): Subscription => {
